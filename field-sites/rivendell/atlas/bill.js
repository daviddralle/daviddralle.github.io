import {createBillCamera} from './bill-camera.js?v=bill-follow-1';
...
export function createBill({scene,pits,pickGround,heightAt,origin,camera,controls,project,host,render,stations,explore}){
 ...
 let enabled=false,paused=matchMedia('(prefers-reduced-motion: reduce)').matches,follow=true,elapsed=0,station=0,dwell=0,measurementElapsed=0,visits=0,pose='Walking',wearing=false;
 ...
 function status(){const text=placingChocolate?(chocolateHover?'Release to drop '+snackName()+' on the ring':'Drag '+snackName()+' over the hillslope'):carrying?(dropPoint?'Dangling · release to drop':'Move Bill over the hillslope · release to put him back'):noticeTime>0?notice:poemActive?(poemTime<3?'Composing a field haiku':'A little hillside poetry'):eating>0?('Eating '+snackName(snackTarget?.kind)):digging?(paused?'Dig paused':`Digging a soil pit · ${Math.min(99,Math.floor(digElapsed/4.2*100))}%`):paused?'Field break':pose==='Walking'?snackTarget?(snackTarget.kind==='almond'?'Almonds spotted!':snackTarget.kind==='dark-chocolate'?'Dark chocolate spotted!':'Milk chocolate spotted!'):`Walking to ${route[station].label}`:`${pose} · stop ${visits}`;$('#bill-status').textContent=text;const remaining=Math.max(0,(boostUntil-performance.now())/1000),boost=Math.ceil(remaining);$('#bill-energy').hidden=!boost;$('#bill-energy-fill').style.transform=`scaleX(${remaining/boostCapacity})`;const meter=$('#bill-energy-meter');meter.setAttribute('aria-valuemax',String(Math.ceil(boostCapacity)));if(meter.getAttribute('aria-valuenow')!==String(boost)){meter.setAttribute('aria-valuenow',String(boost));meter.setAttribute('aria-valuetext',`${boost} seconds remaining · 3× walking · 4× measurements`);$('#bill-energy-time').textContent=`${boost}s`;}host.parentElement.dataset.billBoostSeconds=String(boost);host.parentElement.dataset.billChocolateCount=String(chocolates.filter(f=>f.kind!=='almond').length);host.parentElement.dataset.billAlmondCount=String(chocolates.filter(f=>f.kind==='almond').length);host.parentElement.dataset.billPoemPhase=poemActive?(poemTime<3?'composing':'reading'):'working';host.parentElement.dataset.billState=text;host.parentElement.dataset.billPosition=root.position.toArray().map(x=>x.toFixed(2)).join(',');}
 ...
 const followCamera=createBillCamera({camera,controls,ground:(x,z)=>heightAt(x+origin[0],origin[1]-z)-origin[2]});
 function frameBill(dt,snap=false,moving=true){const view=followCamera.update(root.position,root.rotation.y,dt,{snap,moving});host.parentElement.dataset.billCameraMode=view.mode;host.parentElement.dataset.billTravelGrade=view.grade.toFixed(3);}
 function releaseFollow(){follow=false;$('#bill-follow').setAttribute('aria-pressed','false');}
 function focus(){if(!dwell&&!digging&&!poemActive&&!eating){const target=snackTarget||route[station];if(Math.hypot(target.x-root.position.x,target.z-root.position.z)>.2)root.rotation.y=Math.atan2(target.x-root.position.x,target.z-root.position.z);}frameBill(0,true);}
 ...
 const locked=['#bill-almonds','#bill-chocolate','#bill-dark-chocolate','#bill-dig','#bill-clear-pits','#bill-pause','#bill-follow','#bill-roam','#scene-home','#scene-top','#scene-spin','#scene-vms-detail'];
 function carryUI(){
 ...
 function endCarry(commit){
  ...
  if(dropped){let nearest=0;for(let i=1;i<route.length;i++)if(Math.hypot(route[i].x-root.position.x,route[i].z-root.position.z)<Math.hypot(route[nearest].x-root.position.x,route[nearest].z-root.position.z))nearest=i;station=(nearest+route.length-1)%route.length;visits++;dwell=7;measurementElapsed=0;pose='Checking a field measurement';}
 ...
 const chocolateMaterial=mat('#442016'),wrapperMaterial=mat('#c79556'),darkMaterial=mat('#271510'),darkWrapperMaterial=mat('#624879'),foilMaterial=mat('#e8d8b5');
 function chocolateMesh(kind='chocolate'){const dark=kind==='dark-chocolate',cocoa=dark?darkMaterial:chocolateMaterial,wrapper=dark?darkWrapperMaterial:wrapperMaterial,g=new THREE.Group();box(g,cocoa,0,.08,0,.7,.14,.38);box(g,wrapper,-.17,.085,0,.38,.16,.4);box(g,foilMaterial,.04,.17,0,.06,.025,.39);for(const x of [.14,.3])for(const z of [-.1,.1])box(g,cocoa,x,.175,z,.12,.045,.14);return g;}
 ...
 const chocolates=[];let placingChocolate=false,chocolatePointer=null,chocolateHover=null,chocolatePending=null,chocolateControlsEnabled=true,chocolateCaptureTarget=null,chocolateSource=null,snackTarget=null,eating=0,boostUntil=0,boostCapacity=30,activeSnackKind='chocolate';
 const previewChocolate=new THREE.Group(),previewBar=chocolateMesh(),previewDarkBar=chocolateMesh('dark-chocolate'),previewAlmonds=almondMesh();previewChocolate.add(previewBar,previewDarkBar,previewAlmonds);previewDarkBar.visible=previewAlmonds.visible=false;const chocolateLanding=new THREE.Mesh(new THREE.RingGeometry(.72,.9,48),new THREE.MeshBasicMaterial({color:'#ffe2a0',side:THREE.DoubleSide,transparent:true,opacity:.95,depthTest:false,depthWrite:false}));chocolateLanding.rotation.x=-Math.PI/2;
 ...
 const chocolateButton=$('#bill-chocolate'),darkChocolateButton=$('#bill-dark-chocolate'),almondButton=$('#bill-almonds');
 const snackName=(kind=activeSnackKind)=>kind==='almond'?'almonds':kind==='dark-chocolate'?'dark chocolate':'milk chocolate';
 ...
 function beginChocolate(food=null,kind='chocolate'){
  ...
  activeSnackKind=food?.kind||kind;previewBar.visible=activeSnackKind==='chocolate';previewDarkBar.visible=activeSnackKind==='dark-chocolate';previewAlmonds.visible=activeSnackKind==='almond';chocolateSource=food;
  ...
  host.parentElement.classList.add('chocolate-placing');(activeSnackKind==='almond'?almondButton:activeSnackKind==='dark-chocolate'?darkChocolateButton:chocolateButton).classList.add('snack-held');
  return true;
 }
 ...
 for(const [button,kind] of [[chocolateButton,'chocolate'],[darkChocolateButton,'dark-chocolate'],[almondButton,'almond']]){
 ...
 function dropChocolate(p){if(chocolateSource){const food=chocolateSource;food.mesh.position.copy(p);food.mesh.visible=true;food.x=p.x;food.z=p.z;chocolateSource=null;layoutSnack(food);return;}if(chocolates.length>=8){const oldest=chocolates.find(f=>f!==snackTarget);if(oldest)removeChocolate(oldest);}const mesh=activeSnackKind==='almond'?almondMesh():chocolateMesh(activeSnackKind);mesh.position.copy(p);mesh.rotation.y=.3;overlay.add(mesh);const tag=document.createElement('button');tag.type='button';tag.className='chocolate-label'+(activeSnackKind==='almond'?' almond-label':activeSnackKind==='dark-chocolate'?' dark-chocolate-label':' milk-chocolate-label');tag.textContent=activeSnackKind==='almond'?'':'🍫';tag.title='Hold and drag '+snackName();tag.setAttribute('aria-label','Move '+snackName());host.parentElement.append(tag);const food={mesh,label:tag,x:p.x,z:p.z,kind:activeSnackKind};chocolates.push(food);layoutSnack(food);tag.draggable=false;tag.addEventListener('dragstart',preventNativeDrag);tag.addEventListener('pointerdown',e=>startSnackPointer(e,food));tag.addEventListener('click',e=>e.preventDefault());}

 function finishEating(reward=true){if(!eating)return;const kind=snackTarget?.kind;eating=0;if(snackTarget)removeChocolate(snackTarget);snackTarget=null;notebook.visible=true;rod.visible=true;pose='Walking';if(reward){if(kind==='almond')beginPoem();else{const now=performance.now(),added=kind==='dark-chocolate'?60000:30000;boostUntil=Math.max(now,boostUntil)+added;boostCapacity=(boostUntil-now)/1000;}}}
 ...
 // Wheel/trackpad zoom stays centered on Bill and changes the follow distance.
 // OrbitControls emits the same start/end events for wheel and drag navigation.
 let wheelFollow=false,savedZoomToCursor;
 function finishWheel(){if(!wheelFollow)return;followCamera.adoptZoom(root.position);controls.zoomToCursor=savedZoomToCursor;wheelFollow=false;}
 canvas.addEventListener('wheel',()=>{if(!enabled||!follow||carrying||placingChocolate)return;wheelFollow=true;savedZoomToCursor=controls.zoomToCursor;controls.zoomToCursor=false;setTimeout(finishWheel,0);},{capture:true,passive:true});
 canvas.addEventListener('pointerdown',finishWheel,true);
 controls.addEventListener('start',()=>{if(enabled&&!wheelFollow)releaseFollow();});
 controls.addEventListener('end',finishWheel);
 function update(dt){if(!enabled)return false;updateChocolate(dt);noticeTime=Math.max(0,noticeTime-dt);const old=root.position.clone();if(carrying){carryTime+=dt;if(pendingPointer){moveCarry(pendingPointer.x,pendingPointer.y);pendingPointer=null;}poseCarry();}else if(!paused&&!placingChocolate){elapsed+=dt;if(poemActive){updatePoem(dt);}else if(eating>0){eating-=dt;pose='Eating '+snackName(snackTarget?.kind);arms[0].pivot.rotation.x=-1.6+Math.sin(elapsed*6)*.1;arms[1].pivot.rotation.x=-.8;legs.forEach(l=>l.rotation.x=0);if(snackTarget)snackTarget.mesh.position.copy(root.position).add(new THREE.Vector3(Math.sin(root.rotation.y)*.4,1.5,Math.cos(root.rotation.y)*.4));if(eating<=0){eating=.001;finishEating();}}else if(digging){digElapsed+=dt;const t=Math.min(1,digElapsed/4.2),stroke=Math.sin(digElapsed*Math.PI*2.4);pits.progress(digging,t);root.rotation.x=.12+.12*stroke;root.rotation.z=0;arms[0].pivot.rotation.x=-.5-.3*stroke;arms[1].pivot.rotation.x=-.65-.6*stroke;legs.forEach(l=>l.rotation.x=0);if(t>=1)finishDig();}else if(dwell>0){const workStep=dt*(performance.now()<boostUntil?4:1);dwell-=workStep;measurementElapsed+=workStep;pose=dwell>3?'Checking a field measurement':'Writing in the notebook';arms[0].pivot.rotation.x=-.85;arms[1].pivot.rotation.x=dwell>3?-.35:-1.0+Math.sin(measurementElapsed*7)*.06;legs.forEach(l=>l.rotation.x=0);root.rotation.z=Math.sin(measurementElapsed*2)*.012;if(dwell<=0){station=(station+1)%route.length;pose='Walking';}}else{const target=snackTarget||route[station],dx=target.x-root.position.x,dz=target.z-root.position.z,distance=Math.hypot(dx,dz),step=Math.min(distance,dt*.68*(performance.now()<boostUntil?3:1));if(distance<.2&&snackTarget){eating=2;pose='Eating '+snackName(snackTarget?.kind);rod.visible=false;notebook.visible=false;}else if(distance<.2){visits++;dwell=7;measurementElapsed=0;wearing=visits%2===1;outfit();}else{root.position.x+=dx/distance*step;root.position.z+=dz/distance*step;root.rotation.y=Math.atan2(dx,dz);legs.forEach((l,i)=>l.rotation.x=Math.sin(elapsed*7+i*Math.PI)*.42);arms.forEach((a,i)=>a.pivot.rotation.x=Math.sin(elapsed*7+i*Math.PI+Math.PI)*.24);root.rotation.z=0;}}
 root.position.y=heightAt(root.position.x+origin[0],origin[1]-root.position.z)-origin[2]+(poemActive?-.65:digging||dwell>0||eating>0?0:Math.abs(Math.sin(elapsed*7))*.04);if(follow)frameBill(dt,false,Math.hypot(root.position.x-old.x,root.position.z-old.z)>.00001);}
