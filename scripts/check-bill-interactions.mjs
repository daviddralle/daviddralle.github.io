...
assert(!el('#bill-energy').hidden);assert.equal(el('#bill-energy-meter').getAttribute('aria-valuenow'),'30');assert.equal(el('#bill-energy-fill').style.transform,'scaleX(1)');assert.equal(el('#bill-energy-time').textContent,'30s');
...
assert(Math.abs(Number(el('#bill-energy-fill').style.transform.slice(7,-1))-29.9/30)<1e-7);
...
assert(el('#bill-energy').hidden);assert.equal(el('#bill-energy-fill').style.transform,'scaleX(0)');assert.equal(el('#bill-energy-meter').getAttribute('aria-valuenow'),'0');
